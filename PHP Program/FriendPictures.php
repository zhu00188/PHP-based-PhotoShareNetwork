...
if (isset($_GET['friendId'])) {
    $friendId= $_GET['friendId']; // Set the session variable
    $friendName = getUserById($friendId)['Name'];    
}


$allAlbums = getAllAlbums($friendId); // Fetch user albums
$sharedAlbums = getSharedAlbums($allAlbums);
...
if ($pictureId) {
    ...
    $comments = $pictureId ? getCommentDetails($pictureId) : []; // Assume it's ordered newest to oldest
}
...
if (isset($_POST['addComment'])) {
    ...
    if (!empty($comment) && $pictureId) {
        ...
        // Redirect
        header("Location: FriendPictures.php?friendId=" . $friendId . "&albumId=" . $albumId . "&pictureId=" . $pictureId);
        exit();
    }
}
?>
...
<main class="container w-75">
    <h1 class="text-center my-3">Your Friend <?= htmlspecialchars($friendName) ?>'s Pictures</h1>

    <!-- Album Selection -->
    <form action="FriendPictures.php?friendId=<?= urlencode($friendId) ?>" method="post">
        <div class="mb-3">
...
<?php if ($albumId && !empty($pictures)): ?>
    ...
    <!-- Thumbnails -->
    <div id="thumbnailScrollContainer" class="thumbnail-bar d-flex my-3">
        <div class="thumbnail-scroll-container">
            <?php foreach ($pictures as $picture): ?>
                <form action="FriendPictures.php?friendId=<?= urlencode($friendId) ?>" method="post" class="m-1 d-inline-block">
                    <input type="hidden" name="album" value="<?= htmlspecialchars($albumId) ?>">
